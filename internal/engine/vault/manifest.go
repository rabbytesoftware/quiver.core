...
// getManifest is a generic helper to retrieve any manifest type from disk.
// Returns ErrNotCached if not found, ErrStale if TTL expired.
// On ErrStale, both manifest and path are returned.
...
// putManifest is a generic helper to persist any manifest type to disk.
// Acquires the per-namespace lock for atomic write safety.
...
// deleteManifest is a generic helper to delete any manifest file.
// If one sibling file exists, removes the file only.
// If both siblings are gone, removes the entire namespace directory.
// Idempotent — returns nil if the manifest does not exist.
func deleteManifest(
	s *store,
	ns domain.Namespace,
	filename string,
	...
	err = os.Remove(filePath) // #nosec G304 -- path is sanitised by acquireNamespace()
	if err != nil && !errors.Is(err, os.ErrNotExist) {
		return err
	}

	// Determine the sibling filename
	var siblingFile string
	if filename == arrowFilename {
		siblingFile = quiverFilename
	} else if filename == quiverFilename {
		siblingFile = arrowFilename
	} else {
		// Unknown file type; don't remove directory
		return nil
	}

	siblingPath := filepath.Join(dir, siblingFile)
	if _, err := os.Stat(siblingPath); errors.Is(err, os.ErrNotExist) {
		// Sibling doesn't exist; safe to remove the directory.
		_ = os.RemoveAll(dir) // #nosec G304 -- dir is sanitised by acquireNamespace()
	}
