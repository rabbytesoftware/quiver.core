...
// ─── fake daemon ─────────────────────────────────────────────────────────────
//
// Only the endpoints the surviving dispatch tests below still exercise:
// custom-method dispatch (a WS subscribe plus the mutation POST) and the
// bare-namespace panel's manifest fetch. Every discovery-specific endpoint
// (arrow/collection listing, runtime detail, health, versions) moved with
// the tests that needed them to commands/discovery.
