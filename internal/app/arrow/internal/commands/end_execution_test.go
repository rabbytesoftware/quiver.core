...
func TestEndExecution_Validate_NoExecution_ReturnsError(t *testing.T) {
	cmd := EndExecution{Namespace: "github.com/org/repo"}
...
func TestEndExecution_Validate_WithExecution_ReturnsNil(t *testing.T) {
	cmd := EndExecution{Namespace: "github.com/org/repo"}
...
func TestEndExecution_EmitEvent_UninstallSuccess_SetsAbsent(t *testing.T) {
	...
	rt := &domainRuntime.ArrowRuntime{
		Ref:       "github.com/org/repo",
		Execution: &domainRuntime.Execution{Method: "_uninstall"},
	}
	result := cmd.EmitEvent(rt)
	assert.Equal(t, domain.ArrowStateAbsent, result.State)
	assert.Nil(t, result.Execution)
