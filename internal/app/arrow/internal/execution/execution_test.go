...
func (m *mockRunner) fireHook(
	...
	m.mu.Unlock()
	if h != nil {
		h(ctx, ns, method, execErr, outcome)
	}
}
...
func TestNew_PostExecutionHook_ExecuteCancelled_BeginStop(t *testing.T) {
	axArrow := buildAsynxArrow(t)
	axRuntime := buildAsynxRuntime(t)

	captureRun := &mockRunner{}
	captureInst := &mockInstaller{}

	// Call New to get a wired Execution, then verify hook via direct access.
	// We intercept the wiring by calling New and then directly testing via
	// the internal captureRun/captureInst mocks.
	// Since New builds runner/installer internally, we test hook logic
	// by constructing it manually and setting the hook.
	ns := domain.Namespace("test/arrow")

	// Simulate the hook that New() would wire.
	captureRun.SetPostExecutionHook(func(
		ctx context.Context,
		hookNS domain.Namespace,
		method string,
		execErr error,
		outcome domainRuntime.ExecutionOutcome,
	) {
		...
		case "_execute":
			if errors.Is(execErr, context.Canceled) {
				_ = captureRun.BeginExecution(ctx, hookNS, "_stop", nil)
			}
		case "_uninstall":
			if outcome == domainRuntime.ExecutionOutcomeSuccess {
				_ = captureInst.CleanupAfterUninstall(ctx, hookNS)
			}
		}
	})

	captureRun.fireHook(context.Background(), ns, "_execute", context.Canceled, domainRuntime.ExecutionOutcomeCancelled)

	captureRun.mu.Lock()
	calls := captureRun.beginCalls
	captureRun.mu.Unlock()

	require.Len(t, calls, 1)
	assert.Equal(t, "_stop", calls[0].method)
	assert.Equal(t, ns, calls[0].ns)

	// Ensure CleanupAfterUninstall was NOT called.
	captureInst.mu.Lock()
	assert.Empty(t, captureInst.cleanupCalls)
	captureInst.mu.Unlock()

	_ = axArrow
	_ = axRuntime
}

func TestNew_PostExecutionHook_ExecuteCancelled_NonCancelError_NoStop(t *testing.T) {
	captureRun := &mockRunner{}
	captureInst := &mockInstaller{}
	ns := domain.Namespace("test/arrow")
	...
	nonCancelErr := errors.New("some execution error")
	captureRun.fireHook(context.Background(), ns, "_execute", nonCancelErr, domainRuntime.ExecutionOutcomeFailed)

	captureRun.mu.Lock()
	assert.Empty(t, captureRun.beginCalls)
	captureRun.mu.Unlock()

	_ = captureInst
}

func TestNew_PostExecutionHook_UninstallSuccess_CleanupTriggered(t *testing.T) {
	captureRun := &mockRunner{}
	captureInst := &mockInstaller{}
	ns := domain.Namespace("test/arrow")
	...
	captureRun.fireHook(context.Background(), ns, "_uninstall", nil, domainRuntime.ExecutionOutcomeSuccess)

	captureInst.mu.Lock()
	cleanups := captureInst.cleanupCalls
	captureInst.mu.Unlock()
...
func TestNew_PostExecutionHook_UninstallFailed_NoCleanup(t *testing.T) {
	captureRun := &mockRunner{}
	captureInst := &mockInstaller{}
	ns := domain.Namespace("test/arrow")
	...
	captureRun.fireHook(context.Background(), ns, "_uninstall", errors.New("failed"), domainRuntime.ExecutionOutcomeFailed)
