...
func TestInstall_AlreadyInstalled_IsIdempotent(t *testing.T) {
	// Installing an already-ready arrow is a no-op (202 idempotent).
	// Concurrent installs of the same arrow should not corrupt state.
...
func TestInstall_AlreadyInstalling_IsIdempotent(t *testing.T) {
	// Installing while already installing is a no-op (202 idempotent).
	// The in-flight install will complete normally.
	ns := domain.Namespace("github.com/org/repo")
	manifest := &domain.Arrow{ArrowMeta: domain.ArrowMeta{Name: "A", Version: "1.0.0"}}
