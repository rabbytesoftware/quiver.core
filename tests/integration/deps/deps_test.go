...
func (s *DepsSuite) TestDeps_DiamondDeduplication() {
	...
	for _, fixture := range []string{"dep-diamond/root", "dep-diamond/left", "dep-diamond/right", "dep-diamond/shared"} {
		kit.WaitForState(s.T(), tc, kit.NSFor(fixture, "v1"), domain.ArrowStateReady, 30*time.Second)
	}
...
func (s *DepsSuite) TestDeps_RemoveAfterDependentsGone() {
	...
	kit.WaitForState(s.T(), tc, kit.NSFor("quiver-test/service-b", "v1"), domain.ArrowStateReady, 30*time.Second)

	s.Equal(http.StatusAccepted, tc.Uninstall(kit.NSFor("quiver-test/composed-c", "v1"), nil))
	kit.WaitForState(s.T(), tc, kit.NSFor("quiver-test/composed-c", "v1"), domain.ArrowStateAbsent, 30*time.Second)
	kit.WaitForState(s.T(), tc, kit.NSFor("quiver-test/tool-a", "v1"), domain.ArrowStateAbsent, 30*time.Second)
	kit.WaitForState(s.T(), tc, kit.NSFor("quiver-test/service-b", "v1"), domain.ArrowStateAbsent, 30*time.Second)
